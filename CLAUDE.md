...
**Demo 端**：`python/streamlit_app/demo_v0.py` 是瀏覽器內互動介面。Day 5 CP6 之後重構為 **problem-driven 兩層選單**（Step 1 問題類別 → Step 2 演算法）：目前 active 的是 **Tensor Eigenvalue Problem**（4 個演算法：Multi / HONI / NNI / HONI vs NNI comparison），NLS / NCP 用 🚧 coming-soon placeholder 佔位。Layer 1/2 的 6 個 utility（gaussian_blur + 5 個張量工具）保留在 `_internal/utility_renderers.py` 可 import 但從 UI 隱藏。
...
**Day 5 新增的研究筆記**（memory 之外的新文件）：`docs/papers/rayleigh_quotient_noise_floor_en.md` — NNI 演算法架構 + Rayleigh quotient noise floor 公式推導 + tolerance 選擇實務建議。是 memory 第 10 條的延伸論述版、英文 markdown（KaTeX source 格式）。
...
│   └── streamlit_app/        ★ Streamlit demo (Day 5 CP6 後 problem-driven 結構)
│       ├── demo_v0.py        Router（79 行）、PROBLEM_ALGORITHMS 兩層 dispatch
│       ├── matlab_snippets/  9 個 .m source snippet (utilities + Layer 3)
│       ├── _internal/        UI 隱藏的 Layer 1/2 utilities + 共用 read_snippet
│       │   ├── snippets.py
│       │   └── utility_renderers.py
│       └── problems/
│           └── tensor_eigenvalue/
│               ├── defaults.py     build_q7_tensor
│               └── algorithms.py   Multi / HONI / NNI / HONI-vs-NNI
│
├── docs/
│   ├── algorithms_status.md  三個演算法系統現況 (NNI/HNI/Multi、1 分鐘速查)
│   ├── papers/
│   │   └── rayleigh_quotient_noise_floor_en.md  NNI 研究筆記英文版 (Day 5 新增)
│   └── superpowers/
│       ├── specs/            正式 spec（例：streamlit demo v0 design）
│       └── notes/            工作筆記（例：multi/honi/nni hazard_analysis.md）
│
...
依序跑完這 5 步、然後回報「Day X 收工狀態、下個動作是 Y、要不要開工？」：
...
3. **看 `docs/algorithms_status.md`**（1 分鐘速查三演算法狀態）

4. **若正在做某支 port**：讀對應的 hazard analysis
   ```bash
   ls docs/superpowers/notes/
   # 例如正在做 Multi：
   cat docs/superpowers/notes/multi_hazard_analysis.md
   ```
   若任務涉及 NNI tolerance / noise floor：讀 `docs/papers/rayleigh_quotient_noise_floor_en.md`

5. **需要時主動讀 memory**（不用全讀、按任務需要）
   ```bash
   ls ~/.claude/projects/-Users-csliu-Projects-my-toolbox/memory/
   # 然後用 Read 工具讀相關的幾條
   ```
